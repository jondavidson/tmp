...
class Script:
    ...
    def execute(self, context: Dict[str, Any], execution_mode: str) -> bool:
        if execution_mode == 'process':
            return self._execute_as_process(context)
        elif execution_mode == 'module':
            return self._execute_as_module(context)
        else:
            raise ValueError(f"Unknown execution mode: {execution_mode}")

    @retry(max_retries=3)
    def _execute_as_process(self, context: Dict[str, Any]) -> bool:
        with SCRIPT_EXECUTION_TIME.labels(script_name=self.name).time():
            ...
            # Build the command
            command = [sys.executable, self.name] + args_with_values
    ...
    @retry(max_retries=3)
    def _execute_as_module(self, context: Dict[str, Any]) -> bool:
        with SCRIPT_EXECUTION_TIME.labels(script_name=self.name).time():
            try:
                # Resolve arguments
                args_with_values = self._resolve_arguments(context)
                ...
                if self.date_parameters:
                    args_with_values.extend(['--start_date', context['start_date'], '--end_date', context['end_date']])
                # Prepare to call the script as a module
                module_name = self.name.replace('.py', '').replace('/', '.')
                if module_name.startswith('.'):
                    module_name = module_name[1:]
                logging.info(f"Importing module {module_name}")
                script_module = importlib.import_module(module_name)
                # Assume the script has a main() function we can call
                if hasattr(script_module, 'main'):
                    script_module.main(args_with_values)
                else:
                    logging.error(f"Script {self.name} does not have a main() function")
                    SCRIPT_FAILURES.labels(script_name=self.name).inc()
                    return False
                logging.info(f"Finished {self.name}")
                return True
...
class PipelineExecutor:
    ...
    def _run_script(self, script: Script, context: Dict[str, Any]) -> bool:
        ...
        success = script.execute(context, self.execution_mode)
        with self.lock:
            self.in_progress_scripts.remove(script.name)
        return success


def parse_config(config_path: str) -> Tuple[Dict[str, Dataset], List[Script], str]:
    with open(config_path, 'r') as file:
        config = yaml.safe_load(file)

    # Retrieve execution mode from configuration
    execution_mode = config.get('execution_mode', 'process')
    ...
    return datasets, scripts, execution_mode
...
def main() -> None:
    ...
    datasets, script_list, execution_mode = parse_config('pipeline_config.yaml')
    ...
    for current_date in date_list:
        ...
        executor = PipelineExecutor(scripts_dict, datasets, execution_mode=execution_mode)
